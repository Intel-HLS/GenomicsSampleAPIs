...
class CSVLine:
    ...
    def set(self, attr, value):
        ...
        if attr in self.fields:
            ...
            if attr == 'PLOIDY':
                self.ploidy = value
                self.values[self.fields.index('GT')] = [EMPTYCHAR] \
                    * value
                ...
                if isinstance(value, list):
                    if attr == 'SB':
                        ...
                        self.values[self.fields.index('numFilter')] = \
                            len(value)
                    elif attr == 'ALT':
                        self.numALT = len(value)
                        self.numAD = self.numALT + 1
                        self.numPL = (self.numALT + 1) * (self.numALT
                                + 2) / 2
                        ...
                        if self.numALT == 0:
                            ...
                            raise ValueError('AD[] must be of length {0} but given {1}'.format(str(self.numAD),
                                    value))
                    elif attr == 'AF':
                        if self.numALT == 0:
                            ...
                            raise ValueError('AF[] must be of length {0} but given {1} '.format(str(self.numALT),
                                    value))
                    elif attr == 'AC':
                        if self.numALT == 0:
                            ...
                            raise ValueError('AC[] must be of length {0} but given {1}'.format(str(self.numALT),
                                    value))
                    elif attr == 'PL':
                        if self.numALT == 0:
                            ...
                            raise ValueError('PL[] must be of length {0} but given {1}'.format(str(self.numPL),
                                    value))
                    elif attr == 'GT':
                        if self.ploidy == 0:
                            ...
                            raise ValueError('GT[] must be of length {0} but given {1}'.format(str(self.ploidy),
                                    value))
                    else:
    ...
    def runChecks(self):
        ...
        if not bIsValid:
            returnString += 'SampleId :' + str(SampleId) \
                + ' is invalid\n'
            bPass = False
        ...
        if not bIsValid:
            returnString += 'Location :' + str(Location) \
                + ' is invalid\n'
            bPass = False
        ...
        if not bIsValid:
            ...
            returnString += 'End :' + str(End) + ' < Start :' \
                + str(Location) + '\n'
            bPass = False
    ...
    def getCSVLine(self, clear = True):
        ...
        for index in range(0, len(self.fields)):
            ...
            if self.fields[index] == 'PLOIDY' and self.ploidy == 0:
                value = EMPTYCHAR
            if self.fields[index] in self.arrayFields \
        ...
        for attribute in self.fields:
            ...
            if attribute == 'ALT':
                ...
                if attribute == 'SB':
                    ...
                    num_values = int(values[index])
                if num_values > 0:
                    retValues = self.getRange(values, index + 1,
                            num_values)

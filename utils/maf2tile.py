...
if __name__ == "__main__":
  ...
  parser.add_argument("-s", "--spark", action="store_true", help="Run as spark.")
  ...
  if args.spark:
    ...
    spark_cmd = ["spark-submit", "maf_pyspark.py", "-c", args.config, "-d", args.outputdir, "-o", args.output, "-i"]
    spark_cmd.extend(args.inputs)
    if subprocess.call(spark_cmd) != 0:
      raise Exception("Error running converter")
  else:
    multiprocess_import.parallelGen(args.config, args.inputs, args.outputdir, args.output, args.gzipped)

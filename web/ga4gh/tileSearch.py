...
class GTInt(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_int))) ]

class GTUInt(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_uint))) ]

class GTLong(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_long))) ]

class GTULong(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_ulong))) ]

class GTLongLong(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_longlong))) ]

class GTULongLong(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_ulonglong))) ]

class GTFloat(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_float))) ]

class GTDouble(Structure):
    _fields_ = [ ("name", c_char_p), ("count", c_ulonglong), ("data", (POINTER(c_double))) ]
...
class GTQuery (Structure):
    _fields_ = [('varcount', c_ulonglong), ('nextPageToken', c_char_p), ('VariantArray', POINTER(POINTER(GTCallArray)))]
...
def searchVariants(workspace, arrayName, referenceName, start, end, searchLib, variantSetIds, callSetIds = None, attrList = ['GT', 'REF', 'ALT', 'PL', 'AF', 'AN', 'AC' ], pageSize = -1, pageToken = None):
    ...
    with dbqWrapper.dbquery.getSession() as metadb:
        ...
        tileStart, tileEnd = metadb.contig2Tile(array_idx, referenceName, [start, end])
        qresp = searchLib.query_column(workspace, arrayName, c_ulonglong(tileStart), c_ulonglong(tileEnd), c_ulonglong(token), c_longlong(pageSize), pageToken)
        ...
        for i in range(0, varcount):
            ...
            # Once we get the position, translate back to the chromosome positions before returning data
    ...
    # this will need to be fixed for the new library, so ignoring it for now
            for j in range(0, callcount):
                ...
                unsigned_longlong_count = CallArray[j].contents.unsigned_longlong_count
                string_count = CallArray[j].contents.string_count
                # tile row assignment right now is assuming that callsetid = tilerowid
                CallSetIdx, callId, cname = metadb.tileRow2CallSet(array_idx, CallArray[j].contents.id)
                ...
                if(unsigned_longlong_count>0):
                    unsigned_longlong_arr = CallArray[j].contents.unsigned_longlong_arr
                    unsigned_longlong_elems = gtDecode(unsigned_longlong_count, unsigned_longlong_arr)
...
def searchVariantSets(datasetId = "", pageSize = None, pageToken = None):
    ...
    with dbqWrapper.dbquery.getSession() as metadb:
        ...
        for vs in metadb.datasetId2VariantSets(datasetId):
            ...
            referenceSetId = metadb.referenceSetIdx2ReferenceSetGUID(vs.reference_set_id)

...
def test_searchVariants():
    ...
    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'])
    tmp = res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=[], searchLib=test_search, attrList = current_app.config['FIELDS'])
    tmp = res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["0b8a597e-90ae-40d3-b834-8d4ff108e28e"], searchLib=test_search, attrList = current_app.config['FIELDS'])
    tmp = res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'], pageSize = -2)
    tmp = res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'], pageSize = 0)
    tmp = res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'], pageSize = 1)
    tmp = res.gavresponse_info
    assert tmp == page_res
    ...
    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'], pageToken = 'test_1_115060270_1', pageSize = 1)
    tmp = res.gavresponse_info
    assert tmp == empty_res

    min_res=ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, searchLib=test_search, variantSetIds=["testing"])
    tmp = min_res.gavresponse_info
    assert tmp == gold_res

    res = ts.searchVariants(workspace=current_app.config['WORKSPACE'], arrayName=current_app.config['ARRAYNAME'], referenceName="1", start=1, end=249250621, variantSetIds=["testing"], searchLib=test_search, attrList = current_app.config['FIELDS'], callSetIds = ['f4e59886-325f-4427-91ff-9e9be6635e20'])
    tmp = res.gavresponse_info
    assert tmp == gold_res

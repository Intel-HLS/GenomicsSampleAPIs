...
def variants_search():
    ...
    try:
        ...
        garesp = tileSearch.searchVariants(workspace = current_app.config['WORKSPACE'], arrayName = current_app.config['ARRAYNAME'], variantSetIds = variantSetIds, referenceName = referenceName, start = startWindow, end = endWindow, callSetIds = callSetIds, pageSize=pageSize, pageToken=pageToken, attrList = current_app.config['FIELDS'], searchLib = search_lib)

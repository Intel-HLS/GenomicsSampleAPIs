...
class Reference(_Base):
    ...
    source_accessions = relationship('SourceAccession', secondary='reference_source_accession', backref=backref('reference'))
    #Unique constraint on (reference_set_id, name)
    __table_args__ = (
        sa.UniqueConstraint('reference_set_id', 'name', name='unique_name_per_reference_set_constraint'),
        )
    #Unique index on (reference_set_id, tiledb_column_offset) if tiledb_column_offset non-NULL
    sa.Index('unique_reference_set_id_offset_idx', reference_set_id, tiledb_column_offset, unique=True);
...
sa.event.listen(Reference.__table__, 'after_create', increment_next_column_in_reference_set_sqlite.execute_if(dialect='sqlite'))
...
increment_next_column_in_reference_set_pgsql = sa.DDL('''\
    ...
    '''%(padded_length))
sa.event.listen(Reference.__table__, 'after_create', increment_next_column_in_reference_set_pgsql.execute_if(dialect='postgresql'))

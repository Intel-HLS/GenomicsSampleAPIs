...
def upgrade():
    ...
    op.alter_column('callset', 'id', server_default=sa.Sequence(
        'callset_id_seq').next_value())
    ...
    ''')
    op.add_column(u'callset', sa.Column(
        'source_sample_id', sa.BigInteger, nullable=False))
    op.add_column(u'callset', sa.Column(
        'target_sample_id', sa.BigInteger, nullable=False))
    op.drop_constraint(u'callset_dbrow_id_fkey', 'callset', type_='foreignkey')
    op.drop_constraint(u'callset_individual_id_fkey',
                       'callset', type_='foreignkey')
    op.create_foreign_key('callset_source_sample_id_fkey',
                          'callset', 'sample', ['source_sample_id'], ['id'])
    op.create_foreign_key('callset_target_sample_id_fkey',
                          'callset', 'sample', ['target_sample_id'], ['id'])
    op.drop_column(u'callset', 'dbrow_id')
    op.drop_column(u'callset', 'individual_id')
    op.add_column(u'db_array', sa.Column(
        'num_rows', sa.BigInteger, nullable=False))
...
def downgrade():
    ...
    op.drop_constraint('callset_source_sample_id_fkey',
                       'callset', type_='foreignkey')
    op.drop_constraint('callset_target_sample_id_fkey',
                       'callset', type_='foreignkey')
    op.create_foreign_key(u'callset_individual_id_fkey',
                          'callset', 'individual', ['individual_id'], ['id'])
    op.create_foreign_key(u'callset_dbrow_id_fkey',
                          'callset', 'db_row', ['dbrow_id'], ['id'])
    op.drop_column(u'callset', 'target_sample_id')
    op.drop_column(u'callset', 'source_sample_id')
    op.drop_index('db_array_id_tile_row_id_idx',
                  table_name='callset_to_db_array_association')

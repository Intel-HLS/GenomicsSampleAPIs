...
def process(chromosome, start, end, attributes):
  ...
  for name, uri in masters.items():
    ...
    data = json.loads(data)
    util.log("{1}: Sample Names Received : {0} ".format(api.getSampleNames(data['indices']), name))
    if len(data) > 0:
      sample_index = 0
      print "Example:"
    ...
    util.log("{1}: Total Samples Received : {0} ".format(len(data['indices']), name))
...
if __name__ == '__main__':
  ...
  parser.add_argument("-s", "--start", required=True, type=long, help="start position")
  parser.add_argument("-e", "--end", required=False, type=long, help="end position")
